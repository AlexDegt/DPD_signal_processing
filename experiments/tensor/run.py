...
from model import ParallelRrankCheby2D, ParallelCheby2D
...
model = ParallelRrankCheby2D(order, rank, delays, dtype, device)
...
if param_path != "None":
    model.load_state_dict(torch.load(param_path, map_location=torch.device(device)))
...
# train_type='mnm_lev_marq_manual' # Levenberg-Marquardt on base of Mixed Newton. Hessian and gradient are calculate manually
