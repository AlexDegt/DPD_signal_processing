...
def train_mixed_newton_levenb_marq_manual(model: nn.Module, train_dataset: DataLoaderType, validate_dataset: DataLoaderType, 
    ...
    Hessian and gradient calculation are implemented manually. 
    ...
    """
    # PyTorch must not calculate computational graph
    ...
    # while grad_norm is None or grad_norm >= min_grad_norm:
        ...
        for j, batch in enumerate(train_dataset):
            ...
            with torch.no_grad():
                x, d = batch_to_tensors(batch)
                jac = model.get_jacobian(x)
                e = (d - model(x))[0, 0, :]
                delta_grad = -1 * torch.conj(jac).T @ e
                delta_hess = torch.conj(jac).T @ jac
        ...
        while flag:
            ...
            model.set_flat_params(curr_params)
